@props(['title' => null])
@php
    $temsilci = auth('temsilci')->user();
    $displayName = trim(($temsilci->first_name ?? '').' '.($temsilci->last_name ?? '')) ?: $temsilci->email;
    $initials = $temsilci->first_name
        ? strtoupper(mb_substr($temsilci->first_name, 0, 1).mb_substr($temsilci->last_name ?? '', 0, 1))
        : strtoupper(mb_substr($temsilci->email, 0, 1));
@endphp
<!DOCTYPE html>
...
<head>
    ...
    <title>{{ $title ?? __('temsilci.eyebrow') }} — {{ config('app.name') }}</title>
    ...
    <style>
        ...
        /* ---- Aç/kapa alt menü (ör. Temsilci Bilgileri) ---- */
        .ip-nav-group-btn {
            display: flex;
            align-items: center;
            gap: .7rem;
        ...
        /* ---- Form bileşenleri (x-temsilci.input/label/button/input-error) ---- */
...
<body class="ip-shell">
    <aside class="ip-sidebar">
        <a href="{{ route('temsilci.dashboard') }}" class="ip-brand">
            <span class="ip-brand-mark"><span></span></span>
            <span class="ip-brand-text">
                <span class="ip-brand-eyebrow">TFSF</span>
                <span class="ip-brand-title">{{ __('temsilci.login.heading') }}</span>
            </span>
        </a>

        <nav class="ip-nav">
            <a href="{{ route('temsilci.dashboard') }}" class="ip-nav-item {{ request()->routeIs('temsilci.dashboard') ? 'is-active' : '' }}">
            ...
            <div x-data="{ o: {{ request()->routeIs('temsilci.profile.*') || request()->routeIs('temsilci.password.edit') ? 'true' : 'false' }} }">
                <button type="button" class="ip-nav-group-btn {{ request()->routeIs('temsilci.profile.*') || request()->routeIs('temsilci.password.edit') ? 'is-active' : '' }}" @click="o = !o" :aria-expanded="o.toString()">
                    ...
                    <span class="ip-nav-group-label">{{ __('temsilci.nav.profile') }}</span>
                    <x-temsilci.icon name="chevron-right" class="ip-nav-group-chevron" />
                </button>
                <div class="ip-nav-group-body" x-show="o" x-cloak x-transition>
                    <a href="{{ route('temsilci.profile.edit') }}" class="ip-nav-item {{ request()->routeIs('temsilci.profile.*') ? 'is-active' : '' }}">
                        {{ __('temsilci.nav.profile') }}
                    </a>
                    <a href="{{ route('temsilci.password.edit') }}" class="ip-nav-item {{ request()->routeIs('temsilci.password.edit') ? 'is-active' : '' }}">
                        {{ __('temsilci.nav.password') }}
                    </a>
                </div>
            </div>
        </nav>
    ...
    <div class="ip-main">
        <header class="ip-header">
            ...
            <div class="ip-header-right">
                <nav class="ip-lang" aria-label="{{ __('temsilci.eyebrow') }}">
                    @foreach (config('locales.supported') as $code => $label)
                        <a href="{{ route('temsilci.language', $code) }}" class="{{ app()->getLocale() === $code ? 'is-active' : '' }}">{{ strtoupper($code) }}</a>
                    @endforeach
                </nav>
                ...
                <div x-data="{ open: false }" @click.outside="open = false" style="position: relative;">
                    <button type="button" class="ip-user-btn" @click="open = !open" :aria-expanded="open.toString()">
                        ...
                        <x-temsilci.icon name="chevron-down" />
                    </button>

                    <div class="ip-dropdown" x-show="open" x-cloak x-transition>
                        <div class="ip-dropdown-email">{{ $temsilci->email }}</div>

                        <a href="{{ route('temsilci.profile.edit') }}" class="ip-dropdown-item">
                            <x-temsilci.icon name="account" />
                            {{ __('temsilci.nav.profile') }}
                        </a>
                        ...
                        <form method="POST" action="{{ route('temsilci.logout') }}">
                            ...
                            <button type="submit" class="ip-dropdown-item is-danger">
                                <x-temsilci.icon name="logout" />
                                {{ __('temsilci.nav.logout') }}
                            </button>
                        </form>
                    </div>
                </div>
