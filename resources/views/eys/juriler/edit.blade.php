<x-eys.app-layout :title="__('eys.juri.edit_title')">
    ...
    <div class="ip-card">
        <div class="ip-section-title">{{ __('eys.juri.edit_title') }}</div>

        <form method="POST" action="{{ route('eys.juriler.update', $juri) }}" novalidate autocomplete="off">
            @csrf
            @method('PATCH')
            ...
            <div class="ip-grid-2">
                <div class="ia-field">
                    ...
                    <x-eys.input id="first_name" type="text" name="first_name" :value="old('first_name', $juri->first_name)" autocomplete="off" />
                    <x-eys.input-error :messages="$errors->get('first_name')" />
                </div>
                <div class="ia-field">
                    <x-eys.label for="last_name" :value="__('eys.juri.last_name')" />
                    <x-eys.input id="last_name" type="text" name="last_name" :value="old('last_name', $juri->last_name)" autocomplete="off" />
            ...
            <div class="ip-grid-2">
                <div class="ia-field">
                    ...
                    <x-eys.input id="email" type="email" name="email" :value="old('email', $juri->email)" autocomplete="off" />
                    <x-eys.input-error :messages="$errors->get('email')" />
                </div>
                <div class="ia-field">
                    <x-eys.label for="tckimlikno" :value="__('eys.juri.tckimlikno')" />
                    <x-eys.input id="tckimlikno" type="text" name="tckimlikno" :value="old('tckimlikno', $juri->tckimlikno)" maxlength="11" autocomplete="off" />
            ...
            <div class="ip-grid-2">
                <div class="ia-field">
                    ...
                    <x-eys.input id="phone" type="text" name="phone" :value="old('phone', $juri->phone)" autocomplete="off" />
                ...
                <div class="ia-field">
                    ...
                    <select id="education_level_id" name="education_level_id" class="ia-input">
                        ...
                        @foreach ($educationLevels as $level)
                            <option value="{{ $level->id }}" @selected(old('education_level_id', $juri->education_level_id) === $level->id)>{{ $level->getTranslation()?->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="ia-field" style="margin-bottom: 0;" x-data="{ active: {{ old('status', (int) $juri->status) ? 'true' : 'false' }} }">
