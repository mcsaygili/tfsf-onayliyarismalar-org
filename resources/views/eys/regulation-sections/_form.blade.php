...
<div x-data="{ loc: @js($initialLocale) }">
    <div class="ip-card" style="margin-bottom: 1.5rem;">
        ...
        <div class="ip-grid-2">
            <div class="ia-field" style="margin-bottom: 0;">
                ...
                <x-eys.input id="sort_order" type="number" name="sort_order" min="0" :value="old('sort_order', $section->sort_order)" autocomplete="off" />
                <div style="font-size: .78rem; color: var(--ia-muted-dim); margin-top: .35rem;">{{ __('eys.regulation_section.sort_order_hint') }}</div>
                <x-eys.input-error :messages="$errors->get('sort_order')" />
            </div>
            <div class="ia-field" style="margin-bottom: 0;" x-data="{ active: {{ old('status', (int) $section->status ?: 1) ? 'true' : 'false' }} }">
                <x-eys.label :value="__('eys.regulation_section.status')" />
                <label class="ip-switch">
                    ...
                    <span class="ip-switch-label" x-text="active ? @js(__('eys.regulation_section.status_active')) : @js(__('eys.regulation_section.status_inactive'))"></span>
                </label>
    ...
    <div class="ip-card">
        <div class="ip-section-title">{{ __('eys.regulation_section.name') }}</div>
        ...
        @foreach ($locales as $locale)
            <div x-show="loc === @js($locale)" x-cloak>
                <div class="ia-field" style="margin-bottom: 0;">
                    <x-eys.label :for="$locale.'_name'" :value="__('eys.regulation_section.name')" />
                    <x-eys.input :id="$locale.'_name'" type="text" :name="$locale.'[name]'" :value="old($locale.'.name', $section->getTranslation($locale, false)?->name)" autocomplete="off" />
