...
return new class extends Migration
{
    ...
    {
        if (! Schema::hasColumn('competitions', 'application_starts_at')) Schema::table('competitions', function (Blueprint $table) {
        ...
        if (! Schema::hasColumn('competition_types', 'requires_location')) Schema::table('competition_types', function (Blueprint $table) {
        ...
        foreach (['participant_approval_processes', 'participant_genders', 'age_eligibility_rules', 'member_groups', 'capture_devices'] as $tableName) {
            if (! Schema::hasColumn($tableName, 'is_system')) Schema::table($tableName, function (Blueprint $table) {
                $table->boolean('is_system')->default(false)->after('status');
                $table->unsignedInteger('version')->default(1)->after('is_system');
            });
        }

        if (! Schema::hasColumn('competition_categories', 'member_group_match_mode')) Schema::table('competition_categories', function (Blueprint $table) {
            $table->string('member_group_match_mode', 10)->default('any')->after('age_eligibility_rule_id');
        });

        if (! Schema::hasTable('competition_capture_regions')) Schema::create('competition_capture_regions', function (Blueprint $table) {
        ...
        if (! Schema::hasTable('processing_methods')) Schema::create('processing_methods', function (Blueprint $table) {
            $table->uuid('id')->primary();
        ...
        if (! Schema::hasTable('processing_method_translations')) Schema::create('processing_method_translations', function (Blueprint $table) {
            $table->uuid('id')->primary();
        ...
        if (! Schema::hasColumn('regulation_sections', 'code')) Schema::table('regulation_sections', function (Blueprint $table) {
        ...
        if (! Schema::hasColumn('regulation_items', 'content_type')) Schema::table('regulation_items', function (Blueprint $table) {
        ...
        if (! Schema::hasTable('competition_regulation_inputs')) Schema::create('competition_regulation_inputs', function (Blueprint $table) {
            $table->uuid('id')->primary();
            $table->foreignUuid('competition_id')->constrained()->cascadeOnDelete();
        ...
        if (! Schema::hasTable('competition_regulation_snapshots')) Schema::create('competition_regulation_snapshots', function (Blueprint $table) {
            $table->uuid('id')->primary();
            $table->foreignUuid('competition_id')->constrained()->cascadeOnDelete();
