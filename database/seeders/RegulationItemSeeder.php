...
class RegulationItemSeeder extends Seeder
{
    private const ITEMS = [
        ...
        ['section' => 'jury', 'code' => 'category-evaluation-criterion', 'scope' => 'criterion', 'order' => 20,
            'tr' => '“{{ criterion.category_name }}” kategorisinde “{{ criterion.name }}” kriteri {{ criterion.score_range }} puan aralığında ve {{ criterion.weight }} göreli ağırlıkla değerlendirilecektir.',
            'en' => 'In the “{{ criterion.category_name }}” category, the “{{ criterion.name }}” criterion will be evaluated on a {{ criterion.score_range }} score range with a relative weight of {{ criterion.weight }}.'],
